...
## Part 1. Первые попытки

Перед тем как приступать к помощи коллеге, Джон решил проверить свои знания на совсем небольшой программе.

1. Перед тем, как начать выполнять задание, нужно соеденить свой компьютер (у меня **windows**) c виртуальной машиной (**virtualbox ubuntu 20.04 LTS**)
    - Для этого на **windows** открываем **PowerShell** (найти можно через строку поиска)
    - Также необходимо скопировать с компьютера на виртуальную машину файлы **.sh**  (Нужно полностью прописать путь: **scp "C:\Users\Денис\Desktop\project_S21\DO3_LinuxMonitoring_v1.0.ID_356278-1\src\01\main.sh" nieshays@192.168.0.101:/home/nieshays/src/01/**)
    - **ВАЖНО!!!** Если на основном компьютере не Linux, то эту команду с полной ссылкой нужно прописать в командной строке **ОСНОВНОГО КОМПЬЮТЕРА**. После этого файл скопируется и можно заходить в ВМ и работать уже из нее. 

    ![copy](./Screenshots/Part_8.1.2.png)
    
    - Далее вводим команду **ssh user@ip-адрес_машины** (**user** и **ip-адрес машины** нужно ввести свои. У меня это **ssh nieshays@192.168.0.101**)
    - Вводим пароль от виртуальной машины
    - Потом работаем уже из **PowerShell**, так как онa успешно подключилaсь к виртуальной машине

![PowerShell c ws11](./Screenshots/Part_8.1.0.png)
...
![Вывод](./Screenshots/Part_8.1.1.png)
...
![данные не сохранены](./Screenshots/Part_8.2.1.png) 

![данные сохранены](./Screenshots/Part_8.2.2.png)

![сохраненный файл](./Screenshots/Part_8.2.3.png)
...
- Без ошибок. И фон и текст перекрашиваются в заданные цвета

![color text](./Screenshots/Part_8.3.1.png)

- Здесь показан некорректный ввод и вывод об ошибке. В начале ошибка, что фон и текст не могут быть одного цвета, а второй что аргументов должно быть 4

![error](./Screenshots/Part_8.3.2.png)
...
```
- Здесь я поигрался с цветами и проверил, что все работает

![Part 8.4.1](./Screenshots/Part_8.4.1.png) ![Part 8.4.3](./Screenshots/Part_8.4.3.png)
...
![по-умолчанию](./Screenshots/Part_8.4.2.png)
...
![screept](./Screenshots/Part_8.5.png)